# Ubuntu・Devian(WSL、Win10, Win11)上にGRADs導入
...
あと、↑のbashの処理はいらず、
- gradsをすでにインストールしている状態でXserver(vcxsrv)をアンインストール＆インストールしなおして、
...
## 2.GRADs実行の準備
①ctlファイルの作成。例えばgrib形式ファイルは、GrADsが読めるように以下のように処理をする必要がある。  
また、WRFの出力ファイルをGRADSで読み込むためには、ARWpostというフリーソフトを使った処理が必須。
```
